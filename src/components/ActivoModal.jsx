...
export default function ActivoModal({
  ...
  editMode,
  setEditMode,
  isReadOnly = false // 👈 NUEVA PROP: Por defecto es falso para no romper las otras páginas
}) {
      ...
      <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in duration-300">
        ...
        <div className="relative bg-gradient-to-r from-emerald-600 to-emerald-800 px-10 py-9 text-white">
          <div className="relative z-10 flex justify-between items-center">
            ...
            <button onClick={onClose} className="w-14 h-14 bg-white/10 hover:bg-white/20 text-white rounded-[1.5rem] flex items-center justify-center transition-all group border border-white/10">
        ...
        <div className="overflow-y-auto px-10 py-8 space-y-10 custom-scrollbar bg-slate-50/50">
          ...
          <Section title="Documentación de Origen" icon={<FileText size={18}/>}>
            <Grid>
              <EditableField label="Tipo de Documento" name="tipo_documento" value={localActivo.tipo_documento} editMode={editMode} onChange={handleChange} icon={<FileText size={14}/>} />
              <EditableField label="Referencia / Factura" name="referencia" value={localActivo.referencia} editMode={editMode} onChange={handleChange} icon={<Info size={14}/>} />
              <EditableField label="Folio VR" name="folioVR" value={localActivo.folioVR} editMode={editMode} onChange={handleChange} icon={<Hash size={14}/>} />
              <EditableField label="Fecha del Documento" name="fecha_documento" value={localActivo.fecha_documento} editMode={editMode} onChange={handleChange} icon={<Calendar size={14}/>} />
            </Grid>
          </Section>

          <Section title="Valor e Impacto" icon={<DollarSign size={18}/>}>
            <Grid>
              <EditableField label="Importe (MXN)" name="importe" value={localActivo.importe} editMode={editMode} onChange={handleChange} icon={<DollarSign size={14}/>} isMoney />
              <div className="flex flex-col gap-2 lg:col-span-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 px-1"><Activity size={12}/> Estado Operativo</label>
                <div className={`px-5 py-3 rounded-2xl font-black text-xs uppercase inline-flex w-fit items-center gap-3 border transition-all ${localActivo.actividad == 1 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                  <div className={`w-2.5 h-2.5 rounded-full animate-pulse ${localActivo.actividad == 1 ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                  {localActivo.actividad == 1 ? "Activo en Inventario" : "Inactivo / Baja"}
                </div>
              </div>
            </Grid>
          </Section>
          ...
          <Section title="Descripción Técnica" icon={<Info size={18}/>}>
            {editMode ? (
              <textarea name="descripcion" value={localActivo.descripcion || ""} onChange={handleChange} className="w-full border-2 border-slate-100 rounded-[2.5rem] p-7 min-h-[160px] focus:border-emerald-500 outline-none transition-all font-medium text-slate-700 bg-white shadow-inner" placeholder="Detalla las características..." />
            ) : (
              <div className="bg-white border border-slate-100 rounded-[2.5rem] p-8 min-h-[140px] whitespace-pre-wrap text-slate-600 font-medium leading-relaxed shadow-sm border-b-4 border-b-emerald-500/20">
                {localActivo.descripcion || "No existe una descripción técnica detallada."}
              </div>
            )}
          </Section>
        </div>

        {/* 🟢 FOOTER (Aquí está el cambio clave) */}
        <div className="flex justify-between items-center px-10 py-8 border-t bg-white">
          <button onClick={onClose} className="group flex items-center gap-2 text-slate-400 hover:text-emerald-600 font-black text-[11px] uppercase tracking-[0.2em] transition-all">
            <RotateCcw size={14} className="group-hover:-rotate-45 transition-transform"/> Salir de la ficha
          </button>

          <div className="flex gap-4">
            {/* 🔴 SOLO SI NO ES READ ONLY MOSTRAR BOTONES DE EDICIÓN */}
            {!isReadOnly ? (
              editMode ? (
                <>
                  <button onClick={() => setEditMode(false)} className="bg-slate-100 text-slate-500 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all">
                    Cancelar
                  </button>
                  <button onClick={handleSaveClick} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-100 transition-all flex items-center gap-2 active:scale-95">
                    <Save size={18}/> Guardar Cambios
                  </button>
                </>
              ) : (
                <button onClick={() => setEditMode(true)} className="bg-emerald-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 shadow-xl shadow-emerald-100 transition-all flex items-center gap-2 active:scale-95">
                  <Edit3 size={18}/> Editar Ficha
                </button>
              )
            ) : (
              /* BOTÓN OPCIONAL PARA CUANDO ES SOLO LECTURA */
